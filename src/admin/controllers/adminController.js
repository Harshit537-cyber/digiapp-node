...
exports.updateUserByAdmin = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      if (uploadedFilePath && fs.existsSync(uploadedFilePath)) {
        try {
          ...
          console.error(
            "Failed to unlink local file (user not found):",
            unlinkError.message,
          );
        }
      }
    ...
    if (req.file) {
      ...
      if (!fs.existsSync(req.file.path)) {
        console.error(
          "ERROR: New file does not exist at path before Cloudinary upload:",
          req.file.path,
        );
        return res
          .status(500)
      ...
      if (user.profilePhoto) {
        console.log(
          "Old profile photo found. Attempting to delete from Cloudinary.",
        );
      ...
      console.log(
        "New Cloudinary upload successful. URL:",
      ...
      if (fs.existsSync(req.file.path)) {
        try {
          ...
          console.error(
            "Error deleting local file after Cloudinary upload (update):",
            unlinkError.message,
          );
        }
      } else {
        console.warn(
          "Local file not found for deletion after Cloudinary upload (update), it might have been deleted already.",
        );
      }
    } else if (req.body.profilePhoto === "" && user.profilePhoto) {
      ...
      try {
        await cloudinary.uploader.destroy(`user_profile_photos/${publicId}`);
        console.log(
          "Old Cloudinary image deleted (explicit remove) successfully.",
        );
      } catch (cloudinaryError) {
        console.error(
          "Error deleting old Cloudinary image (explicit remove):",
          cloudinaryError.message,
        );
      }
    ...
    if (error.code === 11000) {
      // Duplicate key error (e.g., mobile)
      return res
        .status(400)
        .json({ message: "Duplicate mobile number", error: error.message });
    }
    res.status(500).json({ message: "Server Error", error: error.message });
  }
};
  ...
  try {
    ...
    if (user.profilePhoto) {
      ...
      try {
        ...
        console.error(
          "Error deleting Cloudinary profile photo:",
          cloudinaryError.message,
        );
      }
    }
...
exports.adminUpdateJob = async (req, res) => {
  ...
  try {
    ...
    if (!job) {
      if (uploadedFilePath && fs.existsSync(uploadedFilePath)) {
        try {
          ...
          console.error(
            "Failed to unlink local job file (job not found):",
            unlinkError.message,
          );
        }
      }
    ...
    if (req.file) {
      ...
      if (!fs.existsSync(req.file.path)) {
        console.error(
          "ERROR: New job file does not exist at path before Cloudinary upload:",
          req.file.path,
        );
        return res
          .status(500)
          .json({
            message: "Server Error: Uploaded job file not found on disk.",
          });
      }

      // If an old job image exists, delete it from Cloudinary
      if (job.jobImage) {
        // Assuming 'jobImage' is the field name in your Job model
        console.log(
          "Old job image found. Attempting to delete from Cloudinary.",
        );
        ...
        try {
          ...
          console.error(
            "Error deleting old Cloudinary job image:",
            cloudinaryError.message,
          );
        }
      }
      ...
      console.log(
        "New Cloudinary job upload successful. URL:",
        updateData.jobImage,
      );
      ...
      if (fs.existsSync(req.file.path)) {
        try {
          ...
          console.error(
            "Error deleting local job file after Cloudinary upload (update):",
            unlinkError.message,
          );
        }
      } else {
        console.warn(
          "Local job file not found for deletion after Cloudinary upload (update), it might have been deleted already.",
        );
      }
    } else if (req.body.jobImage === "" && job.jobImage) {
      // Explicit request to remove existing photo
      ...
      try {
        ...
        console.log(
          "Old Cloudinary job image deleted (explicit remove) successfully.",
        );
      } catch (cloudinaryError) {
        console.error(
          "Error deleting old Cloudinary job image (explicit remove):",
          cloudinaryError.message,
        );
      }
    ...
    if (uploadedFilePath && fs.existsSync(uploadedFilePath)) {
      try {
        ...
        console.error(
          "Failed to unlink local job file in error handler:",
          unlinkError.message,
        );
      }
    }
  ...
  try {
    ...
    if (deletedJob.jobImage) {
      ...
      try {
        ...
        console.error(
          "Error deleting Cloudinary job image:",
          cloudinaryError.message,
        );
      }
    }
